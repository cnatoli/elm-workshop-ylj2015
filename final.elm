...
--
-- Keyboard input -> game specific input
--
...
defaultBall : Ball
defaultBall = { x=0, y=0, vx=defaultVxy, vy=defaultVxy }
...
---
--- Stepping game elements
---

-- step a player forward, making sure it does not fly off the court
stepPlyr : Time -> Direction -> Player -> Bool -> Player
stepPlyr t dir player scored =
    let dir'    = dirToInt dir
        player' = stepObj t { player | vy <- toFloat dir' * defaultVxy }
        y'      = clamp (22-halfHeight) (halfHeight-22) player'.y -- explain clamp
        score'  = player.score + (if scored then 1 else 0)
    in
...
stepGame {paddle1,paddle2,delta}
  ...
  let paddle1' = dirToInt paddle1
      ...
      player1' = stepPlyr delta paddle1 player1 (ball.x > halfWidth)
      player2' = stepPlyr delta paddle2 player2 (ball.x < -halfWidth)
...
display (w,h) {player1, player2, ball} =
    let offset = 10
        scoreText = toString player1.score ++ "    " ++ toString player2.score
                  |> fromString
                  |> Text.height 60
                  |> Text.color textGreen
                  |> Text.typeface ["helvetica"]
                  |> outlinedText { defaultLine | color <- white, width <- 2 }
    ...
    in
      ...
      collage gameWidth gameHeight
              [(rect gameWidth gameHeight)
               ...
               -- Court outline
               (rect (gameWidth - offset) (gameHeight - offset))
                   |> outlined (solid white),
               (circle 40)
                   |> outlined (solid white),
               (segment (0, offset/2 - halfHeight) (0, halfHeight - offset/2))
                   |> traced (solid white),
